...
function Install-DriverUPD {
    ...
    # Coleta todos os INFs que contem o filtroDriver e classifica por tipo de protocolo
    $infCompativeis = foreach ($inf in $arquivosInf) {
        try {
            ...
            if ($conteudo -match [regex]::Escape($filtroDriver)) {
                ...
                $isPCL = ($caminho -match '(?i)(\\|/|_|-)PCL(\\|/|_|-|$)')
                $isPS  = ($caminho -match '(?i)(\\|/|_|-)PS(\\|/|_|-|$)')
                [pscustomobject]@{
                    Arquivo = $inf
                    PCL     = $isPCL
                    PS      = $isPS
    ...
    # Busca o driver registrado apos o pnputil
    # Tenta correspondencia exata primeiro, depois parcial
    # Nao exclui PCL/PS pois o nome registrado vem do INF escolhido e pode conter essas strings legitimamente
    # Exclui apenas o driver Universal generico, que nao e o especifico do modelo
